...
gate("mobile-settings-laptop-wrap", () => evalJson(`(async()=>{window.dispatchEvent(new Event('my-ax:settings-open'));await new Promise(resolve=>setTimeout(resolve,900));const drawer=document.querySelector('aside[aria-label="Settings"]');const drawerRect=drawer?.getBoundingClientRect();if(!drawerRect||Math.round(drawerRect.top)!==0||Math.round(drawerRect.bottom)!==innerHeight)throw new Error('settings is not a full-height drawer');const row=document.querySelector('[data-mcp-id="machinectl"]');const name=row?.querySelector('.laptop-row__name');const tools=row?.querySelector('.laptop-row__tools');if(!row||!name||!tools)throw new Error('laptop row missing');const nameRect=name.getBoundingClientRect();const toolsRect=tools.getBoundingClientRect();if(row.scrollWidth>row.clientWidth)throw new Error('laptop row overflows narrow settings drawer');if(toolsRect.top<=nameRect.top)throw new Error('laptop capability summary did not wrap below name');if([...document.querySelectorAll('footer span')].some(node=>/^\s*session · [0-9a-f]{8}\s*$/i.test(node.textContent||'')))throw new Error('duplicate session id remains in settings');const activeId=document.querySelector('[data-active-session-id]');if(!activeId)throw new Error('minimal active session id missing from app bar');window.dispatchEvent(new Event('my-ax:settings-close'));return JSON.stringify({drawer:{top:Math.round(drawerRect.top),bottom:Math.round(drawerRect.bottom),width:Math.round(drawerRect.width)},row:{scrollWidth:row.scrollWidth,clientWidth:row.clientWidth},wrapped:true,activeSession:activeId.textContent.trim()})})()`));
...
gate("compact-sidebar-controls", () => evalJson(`(async()=>{window.dispatchEvent(new Event('my-ax:sessions-open'));await new Promise(resolve=>setTimeout(resolve,500));const aside=document.querySelector('aside[aria-label="Conversations"]');const button=aside?.querySelector('button[aria-label="New conversation"]');const title=aside?.querySelector('.session-row__title');if(!aside||!button)throw new Error('conversations sidebar controls missing');const rect=button.getBoundingClientRect();const style=getComputedStyle(button);if(rect.width>=180||rect.height>40)throw new Error('new conversation control is too large: '+Math.round(rect.width)+'x'+Math.round(rect.height));if(style.backgroundColor==='rgba(0, 0, 0, 0)'||style.backgroundColor==='transparent')throw new Error('new conversation control is not visually emphasized');if(title&&getComputedStyle(title).textOverflow!=='ellipsis')throw new Error('sidebar does not own title truncation');const firstRow=aside.querySelector('.session-row');if(firstRow&&!/[0-9a-f]{8}/i.test(firstRow.textContent||''))throw new Error('sidebar session short id missing');if(firstRow&&!firstRow.querySelector('a[title="Export markdown"]'))throw new Error('sidebar session export missing');window.dispatchEvent(new Event('my-ax:sessions-close'));return JSON.stringify({button:{width:Math.round(rect.width),height:Math.round(rect.height),background:style.backgroundColor},titleTruncation:title?getComputedStyle(title).textOverflow:null,shortId:true,export:true})})()`));
