...
- Added saved hammers: owner-approved `work_code` recipes stored in D1, runnable against a chosen session through the same Code Mode bridge, with each run recorded as a Run Receipt for Check-in.
- Added Settings CRUD for saved hammers so the owner can create, edit, enable/disable, delete, and test-run Code Mode recipes without treating them as a generic extension marketplace.
...
- Added a searchable Capabilities Settings section explaining built-in and connected tools, memory, execution surfaces, and their authority boundaries, with a link to `/capabilities` for the scoped capability-share proof page.
