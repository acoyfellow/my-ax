...
- **Desk** is the durable board at `/?action=desk`. Agents upsert cards with `desk_upsert` instead of starting a new conversation or flattening Approve/Reject into a push body. A card can report a descriptive status and the agent that owns it without pretending to need a decision. An agent can also ask for free-text input; your answer goes back to that conversation and marks the card answered. Open source stays a real GitLab or GitHub URL, and an optional action stays in the app or opens GitHub/GitLab. `desk_clear` and `DELETE /api/desk` empty the board. A second open desk tab can stay stale until it reloads. A push that only wakes you should use href `/?action=desk`.
- **Live error issues** file, classify, and open a ready PR. A client `pushError` or a server chat error may `POST /api/errors`. One fingerprint is one issue for 24 hours. Desk gets a card. The hook labels and writes one loop board. A 15-minute sweep closes same-fingerprint duplicates and queues missed issues. If `bot/issue-<n>` exists, the Worker opens a ready PR. Review and audit never approve or merge. You merge.
