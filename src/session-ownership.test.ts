...
test("session ownership fails closed when the database check fails", async () => {
  const { env } = envWithResult(null, new Error("D1 unavailable"));
  await assert.rejects(() => requireOwnedSession(env, "session-1", "owner@example.com"), /D1 unavailable/);
});
