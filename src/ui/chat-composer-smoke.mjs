...
// FR-2: a silently-stalled turn (socket healthy, no frames) must retire through
// the normal cancellation path and unlock the owner without cancelling tools.
assertIncludes(chat, "TURN_STALL_MS", "a turn-stall threshold exists");
assertIncludes(chat, "No response from the agent, and no tool is running.", "stalled turn surfaces the recovery notice");
assertIncludes(chat, "lastTurnFrameAt", "the watchdog tracks the last turn frame time");
assertIncludes(chat, "requestActive: !!activeRequestId", "the watchdog only retires an active request");
assertIncludes(chat, "cancelAgent();\n        turnStallSurfaced = true;", "the wired watchdog uses the existing cancellation path");
assertIncludes(chat, "remoteTurnEpoch += 1;", "cancellation retires stale authoritative remote turn fetches");
assertIncludes(chat, "remoteTurn = null;", "cancellation releases a stale remote composer lock");
