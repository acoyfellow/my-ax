...
- desk_upsert writes one card to the owner's durable desk at /?action=desk. Use it whenever work should sit on that board instead of starting a new conversation. It can report a descriptive status and agent owner without asking for a decision. When you need free-text owner input, set reply; the desk sends the answer back to this conversation. Always desk_upsert; if the PWA tab is open and page.listArtifactTools shows setBoard, also invoke that tool; wake with notify_owner href /?action=desk. Do not flatten Approve/Reject into notify_owner body text.
