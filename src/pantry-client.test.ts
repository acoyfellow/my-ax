...
test("pantry hides recipes that use retired capability namespaces", async () => {
  const listEnv = {
    PANTRY_TOKEN: "agent",
    PANTRY: {
      fetch: async () => new Response(JSON.stringify({
        recipes: [
          { name: "legacy", description: "Legacy recipe.", capabilities: ["computer.read"], status: "enabled" },
          { name: "current", description: "Current recipe.", capabilities: ["workspace.read"], status: "enabled" },
        ],
      }), { status: 200 }),
    },
  } as unknown as Env;
  assert.deepEqual((await listPantryRecipes(listEnv)).map((recipe) => recipe.name), ["current"]);
  ...
  const getEnv = {
    ...
    PANTRY: {
      fetch: async () => new Response(JSON.stringify({
        ...
        inputSchema: { type: "object", properties: {} },
        code: "return null;",
        capabilities: ["terrarium.spawn"],
        status: "enabled",
      }), { status: 200 }),
    },
  } as unknown as Env;
  assert.equal(await getPantryRecipe(getEnv, "legacy"), null);
});
