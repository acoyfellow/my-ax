...
export function parseResourceUrl(raw: string): ParsedResource {
  ...
  // ax-mcp: AX-owned MCP portal (<mcp-host>), a separate tenant of the same
  ...
  if (url.hostname === "<mcp-host>") {
    if (url.pathname !== "/mcp") throw new Error("unsupported ax-mcp URL: expected /mcp");
    return { system: "ax-mcp", kind: "ax-mcp.server.tools.list", id: "<mcp-host>/mcp", url: raw.trim() };
  }
