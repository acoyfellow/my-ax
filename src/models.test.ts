...
describe("model catalog", () => {
  it("keeps Workers AI and AI Gateway rows in the full catalog", () => {
    assert.ok(findModel("@cf/moonshotai/kimi-k3"));
  ...
  it("shows gateway rows only when the installation has gateway config", () => {
    assert.deepEqual(availableModels(minimalEnv).map((m) => m.id), ["@cf/moonshotai/kimi-k3", "@cf/zai-org/glm-5.3"]);
    assert.deepEqual(availableModels(gatewayEnv).map((m) => m.id), ["@cf/moonshotai/kimi-k3", "@cf/zai-org/glm-5.3", "gpt-5.6-luna", "gpt-5.6-terra"]);
