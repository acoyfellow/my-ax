...
const JOB_STATUSES = new Set<JobStatus>(["active", "paused", "exhausted"]);
...
function jobRunLimitLabel(job: JobRow): string {
  if (job.max_runs === null) return `${job.run_count} runs consumed · unlimited remaining`;
  const remaining = Math.max(0, job.max_runs - job.run_count);
  return `${job.run_count} of ${job.max_runs} runs consumed · ${remaining} remaining`;
}
...
function renderJobsPage(input: { identityEmail: string; buildId?: string | null; status?: JobStatus; jobs: JobRow[]; statusCounts?: Record<JobStatus, number>; statusCode?: number; error?: string }) {
  ...
  const body = input.error
    ...
    : input.jobs.length
      ? `<ol class="space-y-3">${input.jobs.map((job) => `<li class="rounded-2xl border border-line bg-bg-alt p-4" data-job-list-item="${escapeHtml(job.id)}"><div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 class="text-sm font-semibold text-fg">${escapeHtml(job.name)}</h2><p class="mt-1 line-clamp-3 text-sm leading-6 text-fg-mut">${escapeHtml(job.prompt)}</p><p class="mt-2 text-xs font-medium text-fg-mut">${escapeHtml(jobThreadModeLabel(job))}</p><p class="mt-1 text-xs text-fg-mut">${escapeHtml(jobRunLimitLabel(job))}</p></div><span class="rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${jobStatusClass(job.status)}">${escapeHtml(job.status)}</span></div><div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-fg-mut"><a class="rounded-full bg-brand px-3 py-1.5 font-bold text-white hover:opacity-90" href="/api/jobs/${escapeHtml(job.id)}/history" data-job-history-receipt-href="/api/jobs/${escapeHtml(job.id)}/history">History receipt</a>${job.status === "active" ? `<span>next ${escapeHtml(job.next_run_at)}</span>` : ""}<code class="font-mono text-[10px]">${escapeHtml(job.id)}</code></div></li>`).join("")}</ol>`
...
export function registerJobRoutes(app: Hono<AppEnv>) {
  ...
  const input = (v: Record<string, unknown>): Partial<JobInput> => ({ sessionId: typeof v.sessionId === "string" ? v.sessionId : undefined, threadMode: typeof v.threadMode === "string" ? v.threadMode as RecurringJobThreadMode : undefined, name: typeof v.name === "string" ? v.name : undefined, prompt: typeof v.prompt === "string" ? v.prompt : undefined, cadenceSecs: v.cadenceSecs === undefined ? undefined : Number(v.cadenceSecs), maxRuns: v.maxRuns === undefined ? undefined : v.maxRuns === null ? null : Number(v.maxRuns) });
  ...
  app.get("/jobs", async c => {
    ...
    try {
      ...
      const statusCounts: Record<JobStatus, number> = { active: 0, paused: 0, exhausted: 0 };
