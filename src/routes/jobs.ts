...
function renderJobsPage(input: { identityEmail: string; buildId?: string | null; status?: JobStatus; jobs: JobRow[]; statusCounts?: Record<JobStatus, number>; statusCode?: number; error?: string }) {
  ...
  const counts = input.statusCounts ? `<div class="mt-4 flex flex-wrap gap-2" data-jobs-status-summary>${Array.from(JOB_STATUSES).map((status) => `<a class="rounded-xl border border-line px-3 py-2 text-xs text-fg-mut hover:border-brand hover:text-brand" href="/jobs?status=${status}"><strong class="text-fg">${Number(input.statusCounts?.[status] ?? 0)}</strong> ${status}</a>`).join("")}</div>` : "";
