...
const HIGH_AUTHORITY_PREFIXES = ["machine."] as const;
...
export function isHighAuthorityInlineOnly(capabilities: string[]): boolean {
  ...
  const hostBound = capabilities.some((cap) => {
    ...
    return (ns === "machine" || ns === "workspace") && method !== "none";
  });
  return hostBound;
}
 ...
 *   - the capabilities are NOT high-authority-inline-only (machine-bound code
 *     that the promotion policy will refuse to persist)
 *
