...
const HOST_NAMESPACES = new Set(["machine", "workspace", "computer", "terrarium"]);
...
export function suggestRecipeName(code: string): string {
  ...
  const call = code.match(/\b(?:machine|workspace|computer|terrarium)\.(\w+)/);
...
export function suggestRecipeDescription(code: string, caps: string[]): string {
  const hostCap = caps.find((c) => /^(machine|workspace|computer|terrarium)\./.test(c));
