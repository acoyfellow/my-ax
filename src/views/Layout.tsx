...
function pwaBootScript(buildId?: string, buildTimestamp?: string): string {
  ...
  return '(function(){var current=' + deployed + ',lastCheck=0,checking=false;function q(fn){if(document.readyState===\'loading\')document.addEventListener(\'DOMContentLoaded\',fn,{once:true});else fn();}function apply(u){try{var url=new URL(u||location.href,location.href);var action=url.searchParams.get(\'action\');if(action===\'new\'){sessionStorage.setItem(\'my-ax-start-fresh-once\',\'1\');url.searchParams.delete(\'action\');location.href=url.pathname+(url.search||\'\')+(url.hash||\'\');return;}if(action===\'settings\')q(function(){var f=function(){window.dispatchEvent(new Event(\'my-ax:settings-open\'));};f();setTimeout(f,250);});if(action===\'attention\')q(function(){var f=function(){window.dispatchEvent(new Event(\'my-ax:attention-open\'));};f();setTimeout(f,250);});}catch(e){}}function newer(id,stamp){if(!id||id===current.id)return false;var a=Date.parse(current.timestamp||\'\'),b=Date.parse(stamp||\'\');return !(Number.isFinite(a)&&Number.isFinite(b)&&b<=a);}function hardReload(){try{var u=new URL(location.href);u.searchParams.set(\'_v\',Date.now().toString(36));location.replace(u.pathname+u.search+u.hash);}catch(e){location.reload();}}function check(force){var now=Date.now();if(!current.id||checking||!navigator.onLine||document.visibilityState!==\'visible\'||(!force&&now-lastCheck<60000))return;checking=true;lastCheck=now;fetch(\'/api/version\',{cache:\'no-store\',credentials:\'same-origin\',headers:{\'If-None-Match\':\'"\'+current.id+\'"\'}}).then(function(r){if(r.status===304||!r.ok)return null;var id=r.headers.get(\'X-My-Ax-Version\')||\'\',stamp=r.headers.get(\'X-My-Ax-Version-Timestamp\')||\'\';return newer(id,stamp)?{id:id,timestamp:stamp}:null;}).then(function(next){if(!next)return;current=next;var event=new CustomEvent(\'my-ax:deploy-update\',{cancelable:true,detail:next});if(window.dispatchEvent(event))hardReload();}).catch(function(){}).finally(function(){checking=false;});}if(\'serviceWorker\'in navigator){var swReloaded=false;navigator.serviceWorker.addEventListener(\'controllerchange\',function(){if(swReloaded)return;swReloaded=true;hardReload();});addEventListener(\'load\',function(){navigator.serviceWorker.register(\'/sw.js\',{scope:\'/\',updateViaCache:\'none\'}).then(function(r){r.update();if(r.waiting)r.waiting.postMessage({type:\'my-ax:skip-waiting\'});r.addEventListener(\'updatefound\',function(){var nw=r.installing;if(nw)nw.addEventListener(\'statechange\',function(){if(nw.state===\'installed\'&&navigator.serviceWorker.controller)nw.postMessage({type:\'my-ax:skip-waiting\'});});});}).catch(function(){});});document.addEventListener(\'visibilitychange\',function(){if(document.visibilityState===\'visible\')navigator.serviceWorker.getRegistration().then(function(r){if(r)r.update();}).catch(function(){});});}if(current.id){setTimeout(function(){check(true);},1500);setInterval(function(){check(false);},900000);document.addEventListener(\'visibilitychange\',function(){if(document.visibilityState===\'visible\')check(true);});addEventListener(\'pageshow\',function(ev){if(ev&&ev.persisted)check(true);});addEventListener(\'focus\',function(){check(true);});addEventListener(\'online\',function(){check(true);});}if(\'launchQueue\'in window&&\'LaunchParams\'in window){launchQueue.setConsumer(function(params){var target=params&&params.targetURL;if(target){var url=new URL(target);if(url.pathname!==location.pathname||url.search!==location.search||url.hash!==location.hash){location.href=url.pathname+url.search+url.hash;return;}apply(url.href);}});}apply(location.href);})();';
}

const VIEWPORT_SYNC_SCRIPT = `(function(){var root=document.documentElement,vv=window.visualViewport;if(!vv)return;var baseline=vv.height;function editable(){var e=document.activeElement;return e&&(e.tagName==='INPUT'||e.tagName==='TEXTAREA'||e.isContentEditable);}function sync(){var h=vv.height,isEditable=editable();if(!isEditable&&h>baseline)baseline=h;var keyboardOpen=isEditable&&baseline-h>100;root.classList.toggle('keyboard-open',keyboardOpen);if(!keyboardOpen)root.style.setProperty('--app-h',window.innerHeight+'px');}addEventListener('load',sync,{passive:true});addEventListener('resize',sync,{passive:true});vv.addEventListener('resize',sync,{passive:true});addEventListener('focusin',function(){setTimeout(sync,50)},{passive:true});addEventListener('focusout',function(){root.classList.remove('keyboard-open');},{passive:true});addEventListener('orientationchange',function(){baseline=vv.height;sync()},{passive:true});})();`;
