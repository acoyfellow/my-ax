...
| Capability | Status | Current shape | Evidence |
|---|---:|---|---|
| Unified work surface | ✅ | `work_search` discovers methods; `work_code` dispatches one async program across workspace, Machine, and Cloudbox namespaces. Saved recipes persist owner-approved `work_code` recipes, support list/run reuse and promotion, and record each run as a receipt. Reuse replaces regenerating saved code with exact saved code. Real Workers AI Kimi K2.7 measurements on 2026-06-29 showed about a 72% output-token drop for the repeated procedure; a 2026-06-30 fresh-session attribution run measured 394 output tokens deriving from scratch and 157 via `recipe.run`, while total tokens rose for that tiny procedure because recipe invocation added input/tool overhead. Per-cycle real model usage is instrumented in `cycle_costs` and exposed through owner-scoped `/api/cost-series`. | `src/work-tools.ts`, `src/saved-recipes.ts`, `src/cycle-costs.ts`, `src/routes/cost-series.ts`, `migrations/0013_cycle_cost.sql`, `proof/recipe-dogfood-learning-curve-2026-06-29-reuse.json`, `proof/experiments/overhead-attribution-2026-06-30.json`, `npm run test:recipes`, `npm run test:cycle-costs` |
