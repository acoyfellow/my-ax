...
| Computer preview filesystem | 🧪 | A separate per-owner `ComputerWorkspace` Durable Object exposes bounded `computer.read`, `computer.write`, `computer.list`, and `computer.grep` under its own SQLite `/home/user`. It has no execution backend, does not copy Sandbox data, and Computer-only mutations do not create Sandbox snapshots. It is preview software running in production for dogfood, not a Workspace replacement. | `src/computer-workspace.ts`, `src/workspace-snapshot-classification.ts`, `npm run test:computer` |
