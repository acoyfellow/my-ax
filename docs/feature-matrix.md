...
|---|---:|---|---|
| Check-in | ✅ | `GET /api/check-in` and MCP `my_ax_check_in` summarize unread Attention, running jobs, active run receipts, recent completion, and suggested steers without adding storage. The shell preserves raw API steers while rendering owner destinations for `/attention`, `/runs`, and `/jobs`. | `src/check-in.ts`, `proof/svelte/CheckIn.svelte`, `npm run test:check-in`, `npm run test:owner-routes` |
