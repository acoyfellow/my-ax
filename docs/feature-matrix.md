...
|---|---:|---|---|
| Check-in | ✅ | `GET /api/check-in` and MCP `my_ax_check_in` summarize unread Attention, running jobs, active run receipts, recent completion, and suggested steers without adding storage. The shell preserves raw API steers in `data-check-in-raw-href` while rendering query-preserving owner destinations for `/attention`, `/runs`, and `/jobs`. | `src/check-in.ts`, `proof/svelte/CheckIn.svelte`, `npm run test:check-in`, `npm run test:check-in-shell`, `npm run test:owner-routes` |
| Attention inbox | ✅ | D1 stores owner-scoped items with unread state, href, and optional source session. Push delivery is best effort; the row remains if push fails. Raw `/api/attention` receipts and rendered `/attention` owner views are both owned by `src/routes/attention.ts`; rendered views include stable `data-attention-source-href` source-link markers, `data-attention-api-receipt-href` raw receipt markers, and a same-origin `data-attention-seen-form` that marks the current filtered view seen and returns to the owner page. | `src/routes/attention.ts`, `src/notify.ts`, `npm run test:attention-route`, `npm run test:owner-routes` |
...
| Run receipts | 🧪 | Owner-scoped ledger for explicitly appended events. Saved recipe runs append start/terminal events; it still does not automatically capture every tool call or deploy. Raw `/api/runs` receipts and rendered `/runs` owner views preserve valid status filters, show exact status summaries, expose `data-runs-api-receipt-href`, per-run receipt, and raw event receipt markers, and provide safe next actions back to Check-in or the authoritative API receipt. | `src/run-receipts.ts`, `src/routes/runs.tsx`, `src/routes/recipes.ts`, `npm run test:run-receipts`, `npm run test:owner-routes` |
...
| Capability | Status | Current shape | Evidence |
|---|---:|---|---|
| Unified work surface | ✅ | `work_search` discovers methods; `work_code` dispatches one async program across workspace, Machine, Terrarium, Page, and codemode namespaces. Reusable tools persist owner-approved `work_code` programs through the compatibility recipe store, support list/run reuse, and record each run as a receipt. Only successful code carrying the explicit reusable-tool marker is proposed, and repeated cards collapse in the loaded conversation. Review-first is the default; owners can **Approve & enable** directly from the source-bound chat card or opt into owner-scoped automatic enablement, then review, disable, or delete tools in Settings. Host-bound Machine/Terrarium code remains inline-only in either mode. Reuse replaces regenerating saved code with exact saved code. Real Workers AI Kimi K2.7 measurements on 2026-06-29 showed about a 72% output-token drop for the repeated procedure; a 2026-06-30 fresh-session attribution run measured 394 output tokens deriving from scratch and 157 via `recipe.run`, while total tokens rose for that tiny procedure because recipe invocation added input/tool overhead. Per-cycle real model usage is instrumented in `cycle_costs` and exposed through owner-scoped `/api/cost-series`. | `src/work-tools.ts`, `src/saved-recipes.ts`, `src/cycle-costs.ts`, `src/routes/cost-series.ts`, `migrations/0013_cycle_cost.sql`, `proof/recipe-dogfood-learning-curve-2026-06-29-reuse.json`, `proof/experiments/overhead-attribution-2026-06-30.json`, `npm run test:recipes`, `npm run test:cycle-costs` |
